...
mod tests {
    ...
    use cosmwasm_std::{from_json, Addr, AnyMsg};
    ...
    fn valid_init() {
        ...
        let feebucket_addr = deps.api.addr_make("feebucket");
        let merchant_addr = deps.api.addr_make("merchant");

        // Create valid config state
        let res = instantiate(
            deps.as_mut(),
            env,
            message_info(&feebucket_addr, &[]),
            InitMsg {
                contract_name: "tutorial.sc.pb".into(),
                purchase_denom: "purchasecoin".into(),
                merchant_address: merchant_addr.to_string(),
    ...
    fn invalid_merchant_init() {
        ...
        let err = instantiate(
            deps.as_mut(),
            mock_env(),
            message_info(&Addr::unchecked("merchant"), &[]),
            InitMsg {
                contract_name: "tutorial.sc.pb".into(),
                purchase_denom: "purchasecoin".into(),
    ...
    fn invalid_fee_percent_init() {
        ...
        let err = instantiate(
            deps.as_mut(),
            mock_env(),
            message_info(&Addr::unchecked("feebucket"), &[]),
            InitMsg {
                contract_name: "tutorial.sc.pb".into(),
                purchase_denom: "purchasecoin".into(),
                merchant_address: "merchant".into(),
    ...
    fn query_test() {
        ...
        assert_eq!(resp.merchant_address, merchant_addr);
        assert_eq!(resp.purchase_denom, "purchasecoin");
        assert_eq!(resp.fee_collection_address, feebucket_addr);
    ...
    fn handle_valid_purchase() {
        ...
        let res = execute(
            deps.as_mut(),
            mock_env(),
            message_info(&Addr::unchecked("consumer"), &[coin(100, "purchasecoin")]),
            ExecuteMsg::Purchase {
        ...
        res.messages.into_iter().for_each(|msg| match msg.msg {
            CosmosMsg::Bank(BankMsg::Send {
                ...
                if to_address == merchant_addr.to_string() {
                    assert_eq!(amount[0], expected_transfer)
                } else if to_address == feebucket_addr.to_string() {
    ...
    fn handle_invalid_funds() {
        ...
        let err = execute(
            deps.as_mut(),
            mock_env(),
            message_info(&Addr::unchecked("consumer"), &[]),
            ExecuteMsg::Purchase {
                id: "a7918172-ac09-43f6-bc4b-7ac2fbad17e9".into(),
            },
        )
        ...
        let err = execute(
            deps.as_mut(),
            mock_env(),
            message_info(&Addr::unchecked("consumer"), &[coin(0, "purchasecoin")]),
            ExecuteMsg::Purchase {
                id: "a7918172-ac09-43f6-bc4b-7ac2fbad17e9".into(),
            },
        )
        ...
        let err = execute(
            deps.as_mut(),
            mock_env(),
            message_info(&Addr::unchecked("consumer"), &[coin(100, "fakecoin")]),
            ExecuteMsg::Purchase {
                id: "a7918172-ac09-43f6-bc4b-7ac2fbad17e9".into(),
            },
        )
